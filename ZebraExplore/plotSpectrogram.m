...
if flagSPG
    if flagMd
        if app.deleakedFlag && flagLeak
            temp = squeeze(app.meanSpgDeleaked(app.currentCh,1:app.freqN,1:app.spgl));
        else
            temp = squeeze(app.meanSpg(app.currentCh,1:app.freqN,1:app.spgl));    
        end
    else    
        if app.deleakedFlag && flagLeak
            temp = squeeze(app.spgDeleaked(app.currentCh,app.currentTrial,1:app.freqN,1:app.spgl));
        else
            temp = squeeze(app.spg(app.currentCh,app.currentTrial,1:app.freqN,1:app.spgl));
        end
    end
...
elseif flagBPpower
    if flagMd
        tmp = squeeze(app.meanSpgPlot(app.currentCh,1:app.spgl));
    else
        if (app.deleakedFlag && flagLeak)
            tmp = squeeze(app.spgPlotDeleaked(app.currentCh,app.currentTrial,1:app.spgl));
        else
            tmp = squeeze(app.spgPlot(app.currentCh,app.currentTrial,1:app.spgl));
        end
        if flagFlt
            % adapt the filter frame and order to the length of the data
...
elseif flagEI
    if flagMd
        tmp = squeeze(app.meanEIrat(app.currentCh,1:app.spgl));
    else
        tmp = squeeze(app.EIrat(app.currentCh,app.currentTrial,1:app.spgl));
        if flagFlt
            % adapt the filter frame and order to the length of the data
            frame = int32(app.spgl / 64) * 2 + 1;   % frame must be odd. Thats is why *2+1 !
            if (frame > 3), tmp = sgolayfilt(tmp,3,frame);
